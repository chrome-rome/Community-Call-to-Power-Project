...
function AssignStartingPlots.Create()
	...
	local findStarts = {
		...
		PlaceLobster = AssignStartingPlots.PlaceLobster, -- Added by CCTP
		...
		lobsterData = table.fill(0, iW * iH), -- Added by CCTP - Stores "impact and ripple" data in the lobster layer
		...
		tea_ID, poppy_ID, titanium_ID, aloe_vera_ID, jade_ID, 
		manganese_ID, oak_ID, squid_ID, lobster_ID, amber_ID, tobacco_ID, tin_ID, potato_ID,
		--Additional Bonus Resources
		--berries_ID, flax_ID, barley_ID, mango_ID, wood_ID,--
...
function AssignStartingPlots:__Init()
	...
	for resource_data in GameInfo.Resources() do
		...
		elseif resourceType == "RESOURCE_LOBSTER" then
			self.lobster_ID = resourceID;
...
function AssignStartingPlots:PlaceImpactAndRipples(x, y)
	...
	self:PlaceResourceImpact(x, y, 6, 3) -- Added by CCTP - Lobster layer
		...
		if cs_number <= self.iNumCityStates then -- Make sure it's an active city state before processing.
			if region_number == -1 and iUninhabitedCandidatePlots > 0 then -- Assigned to areas outside of Regions.
				...
				if success == true then
					...
					self:PlaceResourceImpact(cs_x, cs_y, 6, 3) -- Added by CCTP - Lobster layer
...
function AssignStartingPlots:PlaceResourceImpact(x, y, impact_table_number, radius)
	...
	-- Impact #s - 1 strategic - 2 luxury - 3 bonus - 4 fish - 5 city states - 6 natural wonders - 7 marble - 8 sheep - 9 squid - 10 lobster
	local iW, iH = Map.GetGridSize();
	local wrapX = Map:IsWrapX();
	local wrapY = Map:IsWrapY();
	local impact_value = 99;
	...
	elseif impact_table_number == 10 then
		self.lobsterData[impactPlotIndex] = 1;
	end
	if radius == 0 then
		return
	end
...
function AssignStartingPlots:PlaceSpecificNumberOfResources(resource_ID, quantity, amount,
	...
	--
	-- Impact tables: -1 = ignore, 1 = strategic, 2 = luxury, 3 = bonus, 4 = fish, 9 = Squid, 10 = Lobster
...
-- Added by CCTP ----------------------------------------------------------------------------
function AssignStartingPlots:PlaceLobster(frequency, plot_list)
	-- This function places Lobster at members of plot_list. (Sounds lobsery to me!)
	if plot_list == nil then
		print("No lobster were placed! -PlaceLobster");
		return
	end
	local iW, iH = Map.GetGridSize();
	local iNumTotalPlots = table.maxn(plot_list);
	local iNumLobsterToPlace = math.ceil(iNumTotalPlots / frequency /1);
	-- Main loop
	local current_index = 1;
	for place_resource = 1, iNumLobsterToPlace do
		local placed_this_res = false;
		if current_index <= iNumTotalPlots then
			for index_to_check = current_index, iNumTotalPlots do
				if placed_this_res == true then
				...
				local plotIndex = plot_list[index_to_check];
				if self.lobsterData[plotIndex] == 0 then
					local x = (plotIndex - 1) % iW;
					local y = (plotIndex - x - 1) / iW;
					local res_plot = Map.GetPlot(x, y)
					if res_plot:GetResourceType(-1) == -1 then
						-- Placing lobster here. First decide impact radius of this lobster.
						local lobster_radius = Map.Rand(7, "Lobster Radius - Place Lobster LUA");
						if lobster_radius > 5 then
							lobster_radius = 3;
						end
						res_plot:SetResourceType(self.lobster_ID, 1);
						self:PlaceResourceImpact(x, y, 4, lobster_radius);
						placed_this_res = true;
						self.amounts_of_resources_placed[self.lobster_ID + 1] = self.amounts_of_resources_placed[self.lobster_ID + 1] + 1;
					end
				end
			end
		end
...
function AssignStartingPlots:PrintFinalResourceTotalsToLog()
	...
	print("- Lobster....: ", self.amounts_of_resources_placed[self.lobster_ID + 1]);
	...
	print("- Potato....: ", self.amounts_of_resources_placed[self.potato_ID + 1]);
...
function AssignStartingPlots:PlaceStrategicAndBonusResources()
	...
	self:PlaceFish(17 * bonus_multiplier, self.coast_list);
	self:PlaceSquid(14 * bonus_multiplier, self.coast_list);
	self:PlaceLobster(14 * bonus_multiplier, self.coast_list);
	...
	local resources_to_place = {
	{self.potato_ID, 1, 100, 1, 1} };
	self:ProcessResourceList(16 * bonus_multiplier, 3, self.hills_jungle_list, resources_to_place)
	
	local resources_to_place = {
	{self.potato_ID, 1, 100, 1, 1} };
	self:ProcessResourceList(22 * bonus_multiplier, 3, self.hills_open_list, resources_to_place)
