...
--Nuclear Tier 3
UPDATE Technologies
SET Cost = Round(2250 * 1.2, 0) WHERE Tier = 3 AND Era = 'ERA_NUCLEAR';
--Nuclear Tier 2
UPDATE Technologies
SET Cost = Round(2250 * 1.1, 0) WHERE Tier = 2 AND Era = 'ERA_NUCLEAR';
--Nuclear Tier 1
UPDATE Technologies
SET Cost = Round(2250 * 1.0, 0) WHERE Tier = 1 AND Era = 'ERA_NUCLEAR';
...
--Industrial
UPDATE Buildings
SET GoldMaintenance = GoldMaintenance * 1.0 -- 30% Increase was 1.5
...
--Medieval
UPDATE Buildings
SET GoldMaintenance = GoldMaintenance * 1.0 -- 20% Increase was 1.5
WHERE ( PrereqTech IN ( SELECT Type FROM Technologies WHERE Era = 'ERA_MEDIEVAL' ) );

-- Classical
UPDATE Buildings
SET GoldMaintenance = GoldMaintenance * 1.0 -- 20% Increase was 1.2
WHERE ( PrereqTech IN ( SELECT Type FROM Technologies WHERE Era = 'ERA_CLASSICAL' ) );

--Ancient
UPDATE Buildings
SET GoldMaintenance = GoldMaintenance * 1.0 -- 10% Increase was 1.1
...
INSERT INTO GameSpeeds		(ID, Type,					Description,					Help,								DealDuration,	GrowthPercent,	TrainPercent,	ConstructPercent,	CreatePercent,	ResearchPercent,	GoldPercent,	GoldGiftMod,	BuildPercent,	ImprovementPercent, GreatPeoplePercent, CulturePercent, FaithPercent,	BarbPercent,	FeatureProductionPercent,	UnitDiscoverPercent,	UnitHurryPercent,	UnitTradePercent,	GoldenAgePercent,	HurryPercent,	InflationPercent,	InflationOffset,	ReligiousPressureAdjacentCity,	VictoryDelayPercent,	MinorCivElectionFreqMod,	OpinionDurationPercent, SpyRatePercent, PeaceDealDuration,	RelationshipDuration,	PortraitIndex,	IconAtlas)
				 ...
				 SELECT		 0,	 'GAMESPEED_MARATHON',	'TXT_KEY_GAMESPEED_MARATHON',	'TXT_KEY_GAMESPEED_MARATHON_HELP',	90,				450,			300,			300,				300,			450,				300,			67,				300,			300,				300,				375,			450,			400,			300,						375,					375,				300,				200,				100,			5,					-330,				2,								300,					300,						300,					100,			30,					150,					0,				'GAMESPEED_ATLAS'		UNION ALL
				 SELECT		 1,	 'GAMESPEED_EPIC',		'TXT_KEY_GAMESPEED_EPIC',		'TXT_KEY_GAMESPEED_EPIC_HELP',	    45,				225,			135,			135,				150,			225,				150,			75,				150,			225,				150,				188,			188,			150,			150,						188,					188,				150,				125,				100,			10,					-169,				4,								150,					150,						150,					100,			15,					75,						1,				'GAMESPEED_ATLAS'		UNION ALL
				 SELECT		 2,	 'GAMESPEED_STANDARD',	'TXT_KEY_GAMESPEED_STANDARD',	'TXT_KEY_GAMESPEED_STANDARD_HELP',	30,				100,			100,			100,				100,			95,				    100,			100,	        100,			100,				100,				100,			100,			100,			100,						100,					100,				100,				100,				100,			20,					-112,				6,								100,					100,						100,					100,			10,					50,						2,				'GAMESPEED_ATLAS'		UNION ALL
				 SELECT		 3,	 'GAMESPEED_QUICK',		'TXT_KEY_GAMESPEED_QUICK',		'TXT_KEY_GAMESPEED_QUICK_HELP',     25,				84,				67,				67,					67,				84,					67,				125,			67,				84,					67,					84,				84,				67,				67,							84,						84,					67,					80,					100,			35,					-90,				9,								67,						67,							67,						30,				10,					50,						3,				'GAMESPEED_ATLAS';
